...
#define CTL_ALT_STATUS_BUSY (1 << 7)
#define CTL_ALT_STATUS_READY (1 << 6)
#define CTL_ALT_STATUS_ERROR (1 << 0)
#define CTL_DEV 2
...
static inline bool channel_wait(u1 channel, u4 timeout) {
    u2 result;
    while (
        (result = (ctl_reg_read(channel, CTL_ALT_STATUS) & CTL_ALT_STATUS_BUSY))
        && timeout--
        ) {
        msec_wait(1000);
    }
...
#define BYTE_MASK(VALUE) ((VALUE) & 0xFFFFFFFC)
    ...
    print_format("ATA PRIMARY MASTER {s}\n",
        initialize_drive(PRIMARY, MASTER) ? "Present" : "Missing");
#if 0
    print_format("ATA PRIMARY SLAVE {s}\n",
        initialize_drive(PRIMARY, SLAVE) ? "Present" : "Missing");
    print_format("ATA SECONDARY MASTER {s}\n",
        initialize_drive(SECONDARY, MASTER) ? "Present" : "Missing");
    print_format("ATA SECONDARY SLAVE {s}\n",
        initialize_drive(SECONDARY, SLAVE) ? "Present" : "Missing");
#endif
}
