...
fn sum_byte(self: *Self, byte: u8) void {
    self.crc = (self.crc << 8) ^ lookup_table[(self.crc >> 24) ^ byte];
}

pub fn sum_bytes(self: *Self, bytes: []const u8) void {
    for (bytes) |byte| {
        self.sum_byte(byte);
    }
...
pub fn get_result(self: *Self) u32 {
    var n = self.total_size;
    while (n != 0) {
        self.sum_byte(@truncate(u8, n));
        n >>= 8;
    }
    return ~self.crc;
}

pub fn check(data: []const u8) u32 {
    var cksum = Self{};
    cksum.sum_bytes(data);
...
test "Cksum.sum_data" {
    var cksum = Self{};
    cksum.sum_bytes(test_string[0..14]);
    cksum.sum_bytes(test_string[14..]);
    try std.testing.expectEqual(cksum.get_result(), test_sum);
}

test "Cksum.check" {
    try std.testing.expectEqual(check(test_string), test_sum);
}
