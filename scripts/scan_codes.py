...
with open('kernel/platform/ps2_scan_codes.h', 'w') as f:
    print('/* Generated by scripts/scan_codes.py*/', file=f)
    print('#ifndef PS2_SCAN_CODES_HEADER', file=f)
    print('#define PS2_SCAN_CODES_HEADER', file=f)
    for i in codes:
        types = [
        ...
        for code, name, char in types:
            print('#define {0} 0x{1:02x}'.format(name, code), file=f)
            table[code] = char
    print('char ps2_scan_code_chars[256];', file=f)
    print('#endif', file=f)

with open('kernel/platform/ps2_scan_codes.c', 'w') as f:
    print('/* Generated by scripts/scan_codes.py*/', file=f)
    print('#include "ps2_scan_codes.h"', file=f)
    print('\nchar ps2_scan_code_chars[] = {', file=f)
